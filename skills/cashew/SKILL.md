...
On first use or when the user says "install cashew" / "set up cashew", install the automated cron jobs. Read `references/cron-setup.md` for the full job definitions.

**Conservative defaults** (recommended — minimize API cost):
1. **Memory extract** (daily) — checks for new conversation content, extracts to brain
2. **Sleep cycle** (daily) — consolidates knowledge, clusters, evolves hierarchy
3. **DB backup** (daily) — never lose the brain

**Power user mode** (opt-in — for heavy daily use like Bunny):
1. **Memory extract** (every 2hrs)
2. **Sleep cycle** (every 6hrs)
3. **DB backup** (every 6hrs)

Install these via the OpenClaw `cron` tool. Adapt paths and timezone to the user's setup. Start conservative — the user can always increase frequency later.
