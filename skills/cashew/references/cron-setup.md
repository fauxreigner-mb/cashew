...
**Default: conservative mode** — daily jobs to minimize API costs.
For power users who want more frequent updates, change `0 9 * * *` to `0 */2 * * *` (every 2hrs), etc.

## Required: Memory Extract (daily — 9am)

Checks if new conversation content exists since last extraction, extracts to brain only if there's a diff. Power users: change to `0 */2 * * *` for every 2hrs.

```json
{
  "name": "cashew-memory-extract",
  "schedule": {"kind": "cron", "expr": "0 9 * * *", "tz": "USER_TIMEZONE"},
...
## Required: Sleep Cycle (daily — 3am)

Consolidates knowledge, runs clustering, evolves hierarchy. Power users: change to `{"kind": "every", "everyMs": 21600000}` for every 6hrs.

```json
{
  "name": "cashew-sleep-cycle",
  "schedule": {"kind": "cron", "expr": "0 3 * * *", "tz": "USER_TIMEZONE"},
  "sessionTarget": "isolated",
  "delivery": {"mode": "none"},
  "payload": {
    "kind": "agentTurn",
...
## Required: DB Backup (daily — midnight)

Safety net — never lose the brain. Power users: change to `0 */6 * * *` for every 6hrs.

```json
{
  "name": "cashew-db-backup",
  "schedule": {"kind": "cron", "expr": "0 0 * * *", "tz": "USER_TIMEZONE"},
  "sessionTarget": "isolated",
  "delivery": {"mode": "none"},
  "payload": {
    "kind": "agentTurn",
